...
# Download and process charred rice data  ----
# Download from Crema et al 2022 repo: https://github.com/ercrema/yayoi_rice_dispersal
dat  <- read.csv("https://raw.githubusercontent.com/ercrema/yayoi_rice_dispersal/main/data/R14CDB.csv") |> subset(UseForAnalyses == 'TRUE'& C14Age > 1000)

# Cleaining code, adapted from https://github.com/ercrema/yayoi_rice_dispersal/blob/main/data/prepare_data.R
...
dat  <- subset(dat,Region%in%c('Kyushu','ChugokuShikoku','Kansai','Tokai','Chubu'))
...
SiteInfo$area.id <- as.numeric(factor(SiteInfo$Region,levels=c('Kyushu','ChugokuShikoku','Kansai','Tokai','Chubu'),ordered=TRUE))
