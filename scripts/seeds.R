...
fit  <- brm(data=seeds.ratio,family=binomial,rice|trials(impressionsTotal) ~ 1 + (1|SiteID) + Period2 * Region)
summary(fit)
...
text(x=0.7,y=7,label=paste0(sum(subset(seeds.ratio,Region=='Central Highlands'&Period2=='LJ/IY')$impressionsTotal),'(',sum(seeds.ratio$Region=='Central Highlands'&seeds.ratio$Period2=='LJ/IY'),')'))
post.bar.h(subset(pred,region=='Tokai'&period=='Late Jomon to Initial Yayoi')$value,i=6,h=1,col=col1)
text(x=0.06,y=6,label=paste0(sum(subset(seeds.ratio,Region=='Tokai'&Period2=='LJ/IY')$impressionsTotal),'(',sum(seeds.ratio$Region=='Tokai'&seeds.ratio$Period2=='LJ/IY'),')'))
...
text(x=0.12,y=4,label=paste0(sum(subset(seeds.ratio,Region=='Central Highlands'&Period2=='EY')$impressionsTotal),'(',sum(seeds.ratio$Region=='Central Highlands'&seeds.ratio$Period2=='EY'),')'))
post.bar.h(subset(pred,region=='Tokai'&period=='Early Yayoi')$value,i=3,h=1,col=col1)
text(x=1.1,y=3,label=paste0(sum(subset(seeds.ratio,Region=='Tokai'&Period2=='EY')$impressionsTotal),'(',sum(seeds.ratio$Region=='Tokai'&seeds.ratio$Period2=='EY'),')'))
text(x=-0.1,y=3.5,label='Early Yayoi',srt=90)

post.bar.h(subset(pred,region=='Tokai'&period=='Middle Yayoi')$value,i=1,h=1,col=col1)
text(x=1.1,y=1,label=paste0(sum(subset(seeds.ratio,Region=='Central Highlands'&Period2=='MY')$impressionsTotal),'(',sum(seeds.ratio$Region=='Central Highlands'&seeds.ratio$Period2=='MY'),')'))
post.bar.h(subset(pred,region=='Chubu Highlands'&period=='Middle Yayoi')$value,i=0,h=1,col=col2)
text(x=0.8,y=0,label=paste0(sum(subset(seeds.ratio,Region=='Tokai'&Period2=='MY')$impressionsTotal),'(',sum(seeds.ratio$Region=='Tokai'&seeds.ratio$Period2=='MY'),')'))
...
fit.millet  <- brm(data=seeds.freq,family='negbinomial',Millets.abundance ~ 1 + Period2 * Region +  offset(log(N)),iter=3000)
fit.rice  <- brm(data=seeds.freq,family='negbinomial',O.sativa.abundance ~ 1 + Period2 * Region + offset(log(N)),iter = 3000)
